...
-- Trigger para actualizar stock cuando se recibe una orden de compra
DELIMITER $$
CREATE TRIGGER IF NOT EXISTS tr_orden_recibida 
...
-- Trigger para reducir stock cuando se realiza una venta
CREATE TRIGGER IF NOT EXISTS tr_venta_realizada 
...
-- Vista de artículos con información de proveedores predeterminados
CREATE OR REPLACE VIEW v_articulos_proveedores AS
SELECT 
    a.*,
    p.nombre as proveedor_nombre,
    p.telefono as proveedor_telefono,
    p.email as proveedor_email,
    ap.precio_unitario as precio_proveedor,
    ap.demora_entrega
FROM articulos a
LEFT JOIN articulo_proveedor ap ON a.id = ap.articulo_id AND ap.es_predeterminado = TRUE
LEFT JOIN proveedores p ON ap.proveedor_id = p.id;
...
JOIN articulos a ON oc.articulo_id = a.id
