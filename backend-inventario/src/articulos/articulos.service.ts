...
export class ArticulosService {
  ...
  async createArticulo(articulo: CreateArticuloDto): Promise<ArticuloResponseDto> {
    ...
    if (!articuloConProveedor) {
      throw new HttpException('Error al recuperar el artículo creado', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async deleteArticulo(id: number): Promise<{ message: string; articulo: ArticuloResponseDto }> {
    const articuloFound = await this.articuloRepository.findOne({
      where: { id }
    });
  ...
  async updateArticulo(id: number, articulo: UpdateArticuloDto): Promise<ArticuloResponseDto> {
    const articuloFound = await this.articuloRepository.findOne({
      where: {
        id,
        estado: true
    ...
    if (!articuloConProveedor) {
      throw new HttpException('Error al recuperar el artículo actualizado', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async getProveedoresDisponibles(): Promise<ProveedorResponseDto[]> {
    try {
      ...
      return proveedores.map(proveedor => this.toProveedorResponseDto(proveedor));
    } catch (error) {
